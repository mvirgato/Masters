...
struct int_params {double dm; double vel;};
...
int myintegrand(unsigned ndim, const double *x, void *p, unsigned fdim, double *fval){
    ...
    fval[0] = heaviside_product(x[0], x[1], vel) * FD(x[0], x[1], ESCAPE_VEL, dm) * (1 - FD(x[0], x[1], vel, dm));
...
double doing_integral(double dm, double vel){

    double val, error;

    struct int_params params = {dm, vel};
